...
public class Fuse {
    ...
    public func search(_ pattern: Pattern?, in aString: String) -> (score: Double, ranges: [CountableRange<Int>])? {
        ...
        if (pattern.text == text) {
            return (0, [CountableRange<Int>(0..<textLength - 1)])
        }
...
extension Fuse {
    ...
    ///   - aString: The string in which to search for the pattern
    /// - Returns: A tuple containing a `score` between `0.0` (exact match) and `1` (not a match), and `ranges` of the matched characters.
    public func search(_ text: String, in aString: String) -> (score: Double, ranges: [CountableRange<Int>])? {
    ...
    public func search(_ text: String, in aList: [String]) -> [(index: Int, score: Double, ranges: [CountableRange<Int>])] {
        let pattern = self.createPattern(from: text)
        
        var items = [(index: Int, score: Double, ranges: [CountableRange<Int>])]()
