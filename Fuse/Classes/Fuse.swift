...
public class Fuse {
    ...
    public func createPattern(from aString: String) -> Pattern? {
        let pattern = isCaseSensitive ? aString : aString.lowercased()
    ...
    private func _search(_ pattern: Pattern, in aString: String) -> (score: Double, ranges: [CountableClosedRange<Int>]) {
        ...
        var loop = true
        if var index = text.index(of: pattern.text, startingFrom: bestLocation) {
            while loop {
                let distanceFromIndex = text.distance(from: text.startIndex, to: index)
                let score = FuseUtilities.calculateScore(pattern.len,
                                                         errorCount: 0,
                                                         matchLocation: distanceFromIndex,
                                                         loc: location,
                                                         distance: distance)
                threshold = min(threshold, score)
                bestLocation = distanceFromIndex + pattern.len
                if let textIndex = text.index(of: pattern.text, startingFrom: bestLocation) {
                    index = textIndex
                }
                else {
                    loop = false
                ...
                var maskIndex = 0
                while maskIndex < pattern.len {
                    matchMaskArr[distanceFromIndex + maskIndex] = 1
                    maskIndex += 1
                }
            }
        }
        ...
        for indexI in 0 ..< pattern.len {
            ...
            while binMin < binMid {
                if FuseUtilities.calculateScore(pattern.len, errorCount: indexI, matchLocation: location, loc: location + binMid, distance: distance) <= threshold {
                    binMin = binMid
                }
                else {
                    binMax = binMid
            ...
            for indexJ in (start ... finish).reversed() {
                let currentLocation = indexJ - 1
                ...
                let charMatch: Int = {
                    if currentLocation < textCount {
                        ...
                        if
                            let currentIndex = currentLocationIndex,
                            let result = pattern.alphabet[text[currentIndex]]
                        {
                            return result
                        }
                    }
                ...
                bitArr[indexJ] = ((bitArr[indexJ + 1] << 1) | 1) & charMatch

                // Subsequent passes: fuzzy match
                if indexI > 0 {
                    bitArr[indexJ] |= (((lastBitArr[indexJ + 1] | lastBitArr[indexJ]) << 1) | 1) | lastBitArr[indexJ + 1]
                }

                if (bitArr[indexJ] & pattern.mask) != 0 {
                    score = FuseUtilities.calculateScore(pattern.len, errorCount: indexI, matchLocation: location, loc: currentLocation, distance: distance)
            ...
            if FuseUtilities.calculateScore(pattern.len, errorCount: indexI + 1, matchLocation: location, loc: location, distance: distance) > threshold {
                break
            }
